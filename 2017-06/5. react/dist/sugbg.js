...
const Grid_1 = __webpack_require__(64);
const MockupDocsDataProvider_1 = __webpack_require__(66);
const DocsDataProvider_1 = __webpack_require__(65);
...
document.addEventListener("DOMContentLoaded", function (event) {
    ...
    if (wrapperElements.length > 0) {
        ...
        const element2 = new Grid_1.JsGrid("sugbgApp");
...
class JsGrid {
    constructor(domElementId) {
        this._itemTemplate = "<li id='{0}'><a href='javascript:' onclick='{1}'>{2}</a></li>";
        this._elementId = domElementId;
    }
    get dataProvider() { return this._dataProvider; }
    set dataProvider(value) { this._dataProvider = value; }
    RenderData() {
        $('#' + this._elementId).append("<div>Loading data...</div>");
        this._dataProvider.readDocsFromLibrary().then((result) => {
            debugger;
            this._renderIDcosArray(result);
        });
    }
    _renderIDcosArray(data) {
        let itemsAsLiHtml = [];
...
class DocsDataProvider {
    ...
    _getFavoriteDocsForClient_JQuery() {
        ...
        const queryUrlGetAllItems = `${webAbsoluteUrl}/_api/web/Lists/lists(guid'${this._listId}')/GetItems(query=@v1)` +
    ...
    _getFavoriteDocsForClient_fetch() {
        let webAbsoluteUrl = this._webAbsoluteUrl;
